...
class ScheduleApiJobsCommandTest extends \PHPUnit_Framework_TestCase
{
	...
	public function testExecute()
	{
		...
		$this->container->shouldReceive('get')->with('gearman')->andReturn($this->gearman);
		$this->container->shouldReceive('get')->with('doctrine.orm.eveapi_entity_manager')->andReturn($this->entityManager);
		$this->entityManager->shouldReceive('getRepository')->with('TariochEveapiFetcherBundle:ApiCall')->andReturn($this->apiCallRepository);
		$this->apiCallRepository->shouldReceive('loadReadyCalls')->andReturn(array($this->apiCall1));
		...
		$this->assertEquals('Scheduled ' . $apiCallId1 . ': worker ' . $section1 . $name1 . "\n", $commandTester->getDisplay());
	}
